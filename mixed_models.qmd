...
2.  For each additional day of sleep deprivation, reaction time (**increased**/decreased) by **10.4673 ms**.
3.  Bonus: How much variation is explained by the random effect? Hint: (variance of RE) / (total variance). 1378.2/ 1378.2 + 960.5 = about **59%** of the variation in reaction times is due to differences between subjects, while the remaining 41% is unexplained within-subject variability.
...
**Estimate is the same, days is still strong predictor, but there is higher error. This model output looks similar, but things are changed showing that having the mixed effects does make an impact.** **The mixed effect model is preferred since we have multiple observations from the same subject.**
...
2.  Bonus: Convert the log odds to an odds ratio. Hint: *e*^coefficient^ = ? e to the 0.58523

    ```{r}
    exp(0.58523)
    ```

    -   Interpretation: For each additional day of sleep deprivation, the odds of being impaired increases by about **80**%. (1.795 is about 1.80 so increased by 80%)
