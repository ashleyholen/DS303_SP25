...
-   intercept: -43.781
    ...
    -   This is the log-odds of being classified as "versicolor" when the petal length is zero.

    -   not practically meaningful since petal length in data set is positive

-   petal length: 9.002

    -   for each one unit increase in petal length, the log odds of a flower being classified as "virginica" significantly increases compared to "versicolor"

    -   positive coefficient suggests that as petal length increases, the log-odds of being classified as ''virginica" significantly increases compared to "versicolor".

    -   to interpret it in terms of odds, you can exponent-iate the coefficient.

    -   this means that for each additional centimeter of petal length, the odds of a flower being classified as "virginica" compared to "versicolor" increase by a factor of approx. 12,343.6, indicating a very strong relationship.
...
```{r}
flower <- glm(Species ~ Petal.Length + Sepal.Length, data = iris_subset, family = binomial)
summary(flower)
```
...
-   **(Intercept)**: -39.839
    ...
    -   This is the log-odds of the flower being classified as "versicolor" when the petal length and sepal length are zero.

-   **Petal.Length:** 13.313

    -   for each one unit increase in petal length, the log odds of a flower being classified as "virginica" significantly increases compared to "versicolor". This is very much so statistically significant.

-   **Sepal.Length**: -4.017

    -   for each one unit increase in sepal length, the log odds of a flower being classified as "virginica" slightly decreases compared to "versicolor". This holds a small amount of statistical significance.
...
mtcars$am <-as.factor(mtcars$am) #transformed original


# build model
car <- glm(am ~ cyl, data = mtcars, family = binomial)
summary(car)
```
