...
```{r}
model_simple <- lm(price ~ carat, data = train_data)
model_complex <- lm(price ~ depth + table + carat + cut, data = train_data)
...
```{r}
rmse(train_data$price, predict(model_simple, train_data))
rmse(train_data$price, predict(model_complex, train_data))
```

Which is the better model and why?

The complex model preformed better, due to the result of in step 3, we are measuring the RMSE, which measures how well a regression model’s predictions match the actual outcomes. This shows on average, how far off tge predictions are in the same units as your response variable.
